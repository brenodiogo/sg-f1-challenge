...
class ListEventsService(
    ...
    override fun listEvents(
        ...
        page: Int,
        size: Int
    ): PagedResult<EventWithMarket> {
        val sessions = f1EventProvider.getSessions(sessionType, year, country)
        val pageSlice = sessions.drop(page * size).take(size)

        val events = pageSlice.map { session ->
        ...
        return PagedResult(events, page, size, sessions.size)
    }
